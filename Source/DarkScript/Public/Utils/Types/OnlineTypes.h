...
enum class EKickReason : uint8
{
	...
	LevelTooHigh UMETA(DisplayName = "Level Too High"),
...
enum class ESessionJoinError : uint8
{
	...
	PlayerLevelTooHigh UMETA(DisplayName = "Player Level Too High"),
...
inline FText GetKickReasonText(const EKickReason Reason)
{
	...
	case EKickReason::LevelTooHigh:
		return FText::FromString(TEXT("Your level is too high for this session"));
...
inline EKickReason TextToKickReason(const FString& ReasonText)
{
	...
	if (ReasonText == TEXT("Your level is too high for this session"))
		return EKickReason::LevelTooHigh;
