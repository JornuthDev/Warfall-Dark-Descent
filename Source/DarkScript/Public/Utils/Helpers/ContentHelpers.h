...
namespace TablePaths
{
	...
	static const FString Enemies = TEXT("/Script/Engine.DataTable'/DarkScript/Data/Tables/EnemiesTable.EnemiesTable'");
	static const FString EnemiesAbility = TEXT("/Script/Engine.DataTable'/DarkScript/Data/Tables/EnemiesAbilityTable.EnemiesAbilityTable'");
}
...
inline UCurveFloat* GetFloatCurve(const FString& Path)
{
	const FString* PathPtr = &Path;
	if (!PathPtr) return nullptr;
	static ConstructorHelpers::FObjectFinder<UCurveFloat> CurveFloatObj(**PathPtr);
	if (!CurveFloatObj.Succeeded())
	{
		return CurveFloatObj.Object;
	}
	return nullptr;
}
