...
void UHealthSet::Init()
{
	...
	const TArray<FDynamicModifierConfig> Mods{
					...
					{GetHealthRegenRateAttribute(), UCalculation_HealthRegenRate::StaticClass()},
			};
	
	FDynamicEffectConfig Config(Mods);
	Config.DurationPolicy = EGameplayEffectDurationType::Infinite;
	GetDefaultAbilitySystem()->HealthPerPerkEffectHandle = GetDefaultAbilitySystem()->ApplyDynamicEffect(Config);
}
