...
void UFocusSet::Init()
{
	...
	const TArray<FDynamicModifierConfig> Mods{
				...
				{GetFocusRegenRateAttribute(), UCalculation_FocusRegenRate::StaticClass()},
		};
	
	FDynamicEffectConfig Config(Mods);
	Config.DurationPolicy = EGameplayEffectDurationType::Infinite;
	GetDefaultAbilitySystem()->FocusPerPerkEffectHandle = GetDefaultAbilitySystem()->ApplyDynamicEffect(Config);
}
