...
void UStaminaSet::Init()
{
	...
	const TArray<FDynamicModifierConfig> Mods{
			...
			{GetStaminaRegenRateAttribute(), UCalculation_StaminaRegenRate::StaticClass()},
	};
	
	FDynamicEffectConfig Config(Mods);
	Config.DurationPolicy = EGameplayEffectDurationType::Infinite;
	GetDefaultAbilitySystem()->StaminaPerPerkEffectHandle = GetDefaultAbilitySystem()->ApplyDynamicEffect(Config);
}
