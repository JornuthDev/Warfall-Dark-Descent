...
void UPlayerAbilitySystem::GiveExperience(const float Amount, UAbilitySystemComponent* Target)
{
	FDynamicEffect Effect(FDynamicEffectBasic(UPlayerSet::GetExperienceAttribute(), Amount), EGameplayModOp::Additive, true);
	ApplyDynamicGameplayEffect(Effect, nullptr, Target);
}
