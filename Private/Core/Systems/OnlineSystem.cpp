...
void UOnlineSystem::ToFilteredResults(TArray<FSessionSearchResult>& SearchResults, const FSessionFilters& Filters)
{
	...
	for (const FSessionSearchResult& SearchResult : SearchResults)
	{
		...
		if (Filters.Archetypes.Num() > 0)
		{
			...
			{
				const EClassArchetype CurrentPlayerArchetype = UDarkLibrary::GetArchetype(this);
				...
				if (!bCurrentArchetypeIsValid)
				{
					for (const EClassArchetype& FilterArchetype : Filters.Archetypes)
					{
							...
							SearchResult.AvailableArchetypes.Contains(FilterArchetype))
						{
							...
							{
								UDarkLibrary::SetArchetype(this, FilterArchetype);
							}
							break;
						}
					}
	...
	ESessionJoinError& Error)
{
	...
	const int32 PlayerLevel = UDarkLibrary::GetPlayerLevel(this);
	const EClassArchetype PlayerArchetype = UDarkLibrary::GetArchetype(this);
