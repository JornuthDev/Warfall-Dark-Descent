...
void UDefaultAttributeSet::Init(const EClassArchetype Archetype, const FName RowName)
{
	...
	SetAttribute(GetBaseHealthAttribute(), AttributeData->Health);
	SetAttribute(GetBaseHealthRegenRateAttribute(), AttributeData->HealthRegenRate);
	
	SetAttribute(GetBaseManaAttribute(), AttributeData->Mana);
	SetAttribute(GetBaseManaRegenRateAttribute(), AttributeData->ManaRegenRate);
	
	SetAttribute(GetBaseStaminaAttribute(), AttributeData->Stamina);
	SetAttribute(GetBaseStaminaRegenRateAttribute(), AttributeData->StaminaRegenRate);

	SetAttribute(GetBaseFocusAttribute(), AttributeData->Focus);
	SetAttribute(GetBaseFocusRegenRateAttribute(), AttributeData->FocusRegenRate);
	
	SetAttribute(GetBasePhysicDamageMultiplierAttribute(), AttributeData->PhysicDamageMultiplier);
	SetAttribute(GetBaseMagicDamageMultiplierAttribute(), AttributeData->MagicDamageMultiplier);
	SetAttribute(GetBaseAccuracyDamageMultiplierAttribute(), AttributeData->AccuracyDamageMultiplier);

	SetAttribute(GetBaseCriticalChanceAttribute(), AttributeData->CriticalChance);
	SetAttribute(GetBaseCriticalDamageAttribute(), AttributeData->CriticalDamage);
		
	// ───────────────────────────────────────────────────────────────────────────────
	// ─── 2. Initialiser les Resistances (fixes depuis Data) ───────────────────────
	SetAttribute(GetArmorAttribute(), AttributeData->Armor);
	SetAttribute(GetIceResistanceAttribute(), AttributeData->IceResistance);
	SetAttribute(GetFireResistanceAttribute(), AttributeData->FireResistance);
	SetAttribute(GetLightningResistanceAttribute(), AttributeData->LightningResistance);
	SetAttribute(GetHolyResistanceAttribute(), AttributeData->HolyResistance);
	SetAttribute(GetPoisonResistanceAttribute(), AttributeData->PoisonResistance);
	SetAttribute(GetBleedingResistanceAttribute(), AttributeData->BleedingResistance);
}
...
float UDefaultAttributeSet::ConvertDamages(const FGameplayEffectModCallbackData& Data, float Amount) const
{
	...
	FGameplayTag ActiveDamageType;
	
	// Récupérer les tags dynamiques ajoutés lors de l'empowerment
	const FGameplayTagContainer& DynamicTags = Spec.GetDynamicAssetTags();
	
	// Chercher le premier tag qui correspond à un type de dégât connu
	for (const auto& Tag : DamageType::GetList())
	{
		if (DynamicTags.HasTagExact(Tag))
		{
			// Vérifier que ce tag a effectivement une magnitude dans le Spec
			if (Spec.SetByCallerTagMagnitudes.Contains(Tag))
			{
				ActiveDamageType = Tag;
				break;
			}
		}
	}
    ...
    if (AActor* EffectCauser = Context.GetEffectCauser(); EffectCauser && EffectCauser->GetClass()->ImplementsInterface(UAbilitySystemInterface::StaticClass()))
	{
		if (const UAbilitySystemComponent* System = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(EffectCauser))
		{
			ArmorPen = System->GetNumericAttribute(UDamagesAttributSet::GetArmorPenetrationAttribute());
			if (ArmorPen <= 0.0f)
			{
				ArmorPen = 1.f;
			}
    ...
    const TArray<FDamageTypeHandler> DamageHandlers = {
        ...
        },
        {
            ...
            [&]() -> float
            {
                const float Reduction = CalculateArmorReduction(GetArmor(), ArmorPen);
                return FMath::Max(0.f, Amount * (1.0f - Reduction));
            }
        },
	...
	// Si on n'a pas trouvé de type actif, retourner les dégâts non modifiés
	if (!ActiveDamageType.IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("ConvertDamages: No active damage type found in DynamicAssetTags"));
		return Amount;
	}
	
	// Appliquer la réduction correspondant au type de dégât actif
	for (const auto& [Tag, CalculateReduction] : DamageHandlers)
	{
		if (Tag == ActiveDamageType)
		{
			UE_LOG(LogTemp, Warning, TEXT("ConvertDamages: Applying resistance for %s"), *Tag.ToString());
...
void UDefaultAttributeSet::EmpoweredMagnitude(FGameplayEffectSpec& Spec, const FGameplayTag& DamageTypeTag, const bool bIsCritic) const
{
	...
	if (AActor* EffectCauser = Context.GetEffectCauser())
	{
		if (EffectCauser->GetClass()->ImplementsInterface(UAbilitySystemInterface::StaticClass()))
		{
			if (const UAbilitySystemComponent* System = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(EffectCauser))
			{
				if (const UDamagesAttributSet* DamagesAttr = const_cast<UDamagesAttributSet*>(System->GetSet<UDamagesAttributSet>()))
				{
					BaseMagnitude = DamagesAttr->GetFinalDamage(DamageTypeTag);
				}
			}
		}
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("EmpoweredMagnitude:: Before %f"), BaseMagnitude);
			 ...
			 DamageTypeTag == DamageType::Lightning || DamageTypeTag == DamageType::Holy)
	{
		Multiplier = GetMagicDamageMultiplier();
			 ...
			 DamageTypeTag == DamageType::Ranged)
	{
		Multiplier = GetAccuracyDamageMultiplier();
	}
	float Result = BaseMagnitude * Multiplier;
	const float CritChance = GetCriticalChance();
	const float CritDamage = GetCriticalDamage();
	
	
	UE_LOG(DarkScript, Warning, TEXT("EmpoweredMagnitude:: After %f"),Result);
	if (bIsCritic)
	{
		Result *= CritDamage;
		UE_LOG(DarkScript, Warning, TEXT("EmpoweredMagnitude:: CRITICAL HIT! (%.2f%% chance) - Damage: %f -> %f"), 
			CritChance * 100.f, BaseMagnitude * Multiplier, Result);
	}
	
	Spec.SetSetByCallerMagnitude(DamageTypeTag, Result);
	Spec.AddDynamicAssetTag(DamageTypeTag);
}
...
void UDefaultAttributeSet::SetAttribute(const FGameplayAttribute& Attribute, const float Value) const
{
	if (GetOwningAbilitySystemComponent())
	{
		GetOwningAbilitySystemComponent()->SetNumericAttributeBase(Attribute, Value);
	}
}
...
void UDefaultAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION_NOTIFY(UDefaultAttributeSet, BaseCriticalHeal, COND_None, REPNOTIFY_Always);
	// ───────────────────────────────────────────────────────────────────────
	// HEALTH
	// ───────────────────────────────────────────────────────────────────────
	...
	DOREPLIFETIME_CONDITION_NOTIFY(UDefaultAttributeSet, CriticalHealBonus, COND_None, REPNOTIFY_Always);
	// ───────────────────────────────────────────────────────────────────────
	// RESISTANCE
	// ───────────────────────────────────────────────────────────────────────
...
void UDefaultAttributeSet::OnRep_BaseCriticalHeal(const FGameplayAttributeData& OldBaseCriticalHeal)
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UDefaultAttributeSet, BaseCriticalHeal, OldBaseCriticalHeal);
}
...
// ═══════════════════════════════════════════════════════════════════════════
// CALLBACKS DE RÉPLICATION - CRITICAL HEAL
// ═══════════════════════════════════════════════════════════════════════════

void UDefaultAttributeSet::OnRep_CriticalHeal(const FGameplayAttributeData& OldCriticalHeal)
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UDefaultAttributeSet, CriticalHeal, OldCriticalHeal);
}

void UDefaultAttributeSet::OnRep_CriticalHealBonus(const FGameplayAttributeData& OldCriticalHealBonus)
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UDefaultAttributeSet, CriticalHealBonus, OldCriticalHealBonus);
}
...
void UDefaultAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	// ───────────────────────────────────────────────────────────────────────
	if (Attribute == GetApplyDamagesAttribute() && GetApplyDamages() != 0)
	{
		float Damages = GetApplyDamages() == 0 ? 0.f : FMath::Abs(GetApplyDamages());
		UE_LOG(LogTemp, Warning, TEXT("Receive Damages Before: %f"), -Damages);
		SetApplyDamages(0.f);
		Damages = -FMath::Abs(ConvertDamages(Data, Damages));
		UE_LOG(LogTemp, Warning, TEXT("Receive Damages After: %f"), Damages);
